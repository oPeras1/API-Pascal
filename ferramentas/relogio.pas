var respostahoras,stringdia:string;
    numeros:array[1..9,1..6] of string;
    ...
    repeat
      ...
      while true do
        begin
          ...
          clrscr;
          stringdia:='Hoje é '+LongDayNames[DayOfWeek(Date)]+', dia '+VarToStr(day)+' de '+LongMonthNames[month]+' do ano '+VarToStr(year);
          GotoXY(((WindMaxX - Length(stringdia)) div 2),WhereY);
          writeln(stringdia);
          stringdia:='São '+VarToStr(hr)+' h, '+VarToStr(min)+' min, '+VarToStr(sec)+' sec, '+VarToStr(ms)+' ms';
          GotoXY(((WindMaxX - Length(stringdia)) div 2),WhereY);
          writeln(stringdia);
          Delay(1000);
        end;
      segundo:=VarToStr(sec);
      mssegundo:=VarToStr(ms);
      readkey;
